...
st.title("🔱 大黒天AI V6 - 完全無欠・超ガバガバ実戦版")
...
def parse_pasted_text(text):
    ...
    # パターン1: 行ごとに「馬番」と「馬名」を拾う
    for line in lines:
        line = line.strip()
        if not line: continue
        
        # もし行が「1〜18の数字だけ」なら馬番として記憶
        ...
        # もし行が「1 イクイノックス」のように数字と文字がくっついていたら
        ...
        # カタカナ2文字以上の馬名っぽい行を見つけたら、記憶している馬番と合体
    ...
    # パターン2: 上の処理で1頭も拾えなかった時の「最終奥義（無理やり抽出）」
    if not horses_data:
        # テキスト全体からカタカナの塊を全部ぶっこ抜く
        ...
        # 重複を消して、出てきた順に馬番1, 2, 3...と強制的に割り当てる
        seen = set()
        unique_names = []
...
if st.sidebar.button("🔱 予想・期待値計算を実行"):
    if not models:
    ...
    with st.spinner("🏇 意地でもデータを抽出 ＆ 裏APIから最新オッズを取得中..."):
    ...
    if df_race is None or df_race.empty:
        st.error("❌ さすがに抽出不可能なレベルのテキストです。もう一度コピペしてみてください。")
    else:
        # APIから取れたオッズを結合（取れなければ10.0倍、1.1倍と仮定）
        ...
        # 💎 複勝期待値（EV）の計算 ＝ (AI複勝確率 × 複勝オッズ下限)
        df_race['複勝期待値(EV)'] = df_race['AI複勝率_num'] * df_race['複勝オッズ_下限']
        
        def get_rank(ev):
            if ev >= 1.25: return "👑 Sランク"
            elif ev >= 1.15: return "🔥 Aランク"
            elif ev >= 1.10: return "👍 Bランク"
            return "-"
        df_race['おすすめ度'] = df_race['複勝期待値(EV)'].apply(get_rank)
        ...
        st.write(f"### 🏆 お宝馬券ランキング（期待値順）")
        df_final = df_race[['馬番', '馬名', '単勝オッズ', '複勝オッズ_下限', 'AI複勝率', '複勝期待値(EV)', 'おすすめ度']].copy()
        df_final['複勝期待値(EV)'] = df_final['複勝期待値(EV)'].map('{:.2f}'.format)
        st.dataframe(df_final.sort_values(by='複勝期待値(EV)', ascending=False), use_container_width=True, hide_index=True)
        
        with st.expander("🕵️ 五大老AI 衆議（個別予測）"):
