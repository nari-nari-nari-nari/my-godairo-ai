    ...
    if not target_url:
        ...
        with st.spinner("🌐 netkeibaから出馬表とオッズを全自動解析中..."):
            try:
                ...
                # Pandasの 'bs4' パーサーで読み込む
                tables = pd.read_html(io.StringIO(html_text_clean), flavor='bs4')
                df_live = max(tables, key=len)
                
                if isinstance(df_live.columns, pd.MultiIndex):
                    df_live.columns = ['_'.join(map(str, c)).strip() for c in df_live.columns]
                else:
                ...
                if race_id_match:
                    ...
                    try:
                        ...
                        odds_tables = pd.read_html(io.StringIO(str(odds_soup)), flavor='bs4')
                        df_odds_scraped = None
                        for tbl in odds_tables:
                            if isinstance(tbl.columns, pd.MultiIndex):
                                tbl.columns = ['_'.join(map(str, c)).strip() for c in tbl.columns]
                            else:
                                tbl.columns = [str(c).strip() for c in tbl.columns]
                            
                            col_check = "".join(tbl.columns)
                            if '馬番' in col_check and ('単勝' in col_check or '複勝' in col_check or 'オッズ' in col_check):
                                df_odds_scraped = tbl
                                break
                        ...
                        if df_odds_scraped is not None:
                            ...
                            if c_num is not None:
                                ...
                                # 🌟複勝オッズ（下限）の取得！ 「1.5 - 2.1」のような範囲から「1.5」を正規表現で引っこ抜く
...
if st.session_state.scraped_data is not None:
    ...
    # 【非線形複勝率キャリブレーション】
    # 単純な「単勝確率の3倍」という歪んだ予測を廃止し、ハルヴィル近似モデルを適用。
    ...
    for name, pred_val in preds.items():
        ...
        df_display[f"{name[:4]}予測"] = (np.clip(ind_place_prob, 0, 1) * 100).map('{:.1f}%'.format)
