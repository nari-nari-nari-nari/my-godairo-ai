...
if st.sidebar.button("🔱 予想・ケリー計算を実行"):
    ...
    if df_race is None or df_race.empty:
        ...
        with st.expander("📊 すべての馬連・ワイドの期待値(EV)一覧を見る（買わない馬券のチェック用）"):
            if not df_all_pairs.empty:
                df_all_pairs['期待値(EV)'] = df_all_pairs['期待値(EV)'].map('{:.2f}'.format)
                df_all_pairs['確率'] = (df_all_pairs['確率'] * 100).map('{:.1f}%'.format)
                # 期待値が高い順にソートして表示
                df_all_pairs = df_all_pairs.sort_values(by=['券種', '期待値(EV)'], ascending=[False, False])
