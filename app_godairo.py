    ...
    try:
        ...
        for row in soup.select('.HorseList'):
            try:
                ...
                continue
        return pd.DataFrame(horses_data) if horses_data else None, course_type, distance
    except:
        return None, "芝", 1600
...
url_input = st.sidebar.text_input("ネット競馬の出馬表URL", "https://race.netkeiba.com/race/shutuba.html?race_id=202606010101")
st.sidebar.markdown("---")
...
if st.sidebar.button("🔱 予想を実行"):
    ...
    with st.spinner("🏇 ネット競馬から出馬表とコース情報をハッキング中..."):
        df_race, fetched_course, fetched_dist = fetch_race_data(race_id)
        
    if df_race is None or df_race.empty:
        st.error("出馬表の取得に失敗しました。URLが正しいか確認してください。")
    else:
        st.success(f"✅ {len(df_race)}頭の出走データと、レース条件（{fetched_course} {fetched_dist}m）を自動取得しました！")
