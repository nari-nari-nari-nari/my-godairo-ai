...
if st.sidebar.button("🔱 予想・ケリー計算を実行"):
    ...
    with st.spinner("🏇 データを抽出＆全券種のオッズを取得・計算中..."):
    ...
    if df_race is None or df_race.empty:
        ...
        # 🏁 レース番号を抽出してドカンと表示
        race_num_disp = "??"
        if race_id and len(str(race_id)) >= 2 and str(race_id).isdigit():
            race_num_disp = str(int(str(race_id)[-2:])) + "R"
            
        st.markdown(f"<h2 style='text-align: center; color: #ffb44b;'>🏁 【{venue} {race_num_disp}】 大黒天AI 予想結果</h2>", unsafe_allow_html=True)
        st.markdown("---")
        ...
        for i in range(n):
            for j in range(i+1, n):
                ...
                if q_odds > 0:
                    ...
                    q_ev = q_prob * q_odds
                    # 馬連は EV 1.15 以上を買い目とする
                    if q_ev >= 1.15:
                        b_val = q_odds - 1.0
                        k_frac = max(0, (q_prob * b_val - (1 - q_prob)) / b_val)
                        k_amt = int(budget * min(k_frac, 1.0) * 0.25) # 馬連はリスク高めなので四半ケリー
                        k_amt = (k_amt // 100) * 100
                        if k_amt > 0:
                ...
                if w_odds > 0:
                    w_prob = ai_wide[i, j]
                    w_ev = w_prob * w_odds
                    # ワイドも EV 1.15 以上を買い目とする
                    if w_ev >= 1.15:
                        b_val = w_odds - 1.0
                        k_frac = max(0, (w_prob * b_val - (1 - w_prob)) / b_val)
                        k_amt = int(budget * min(k_frac, 1.0) * 0.25) # 四半ケリー
                        k_amt = (k_amt // 100) * 100
                        if k_amt > 0:
        ...
        # 画面表示
        # ----------------------------------------------------
        st.write(f"### 🏆 複勝 期待値＆ケリー推奨（予算: {budget:,}円）")
        ...
        if not df_pairs.empty:
            st.write(f"### 🔗 馬連・ワイド 期待値1.15超え 激アツ推奨買い目")
            ...
            st.info("ℹ️ 現在のオッズでは、馬連・ワイドで期待値(EV)が1.15を超え、購入対象となる組み合わせはありませんでした。")
